...
class Form extends Component {
  renderField(data) {
    ...
    return <div className={`form-group${hasError ? ' has-error' : ''}`}>
      <label htmlFor={`user_${data.input.name}`} className="control-label">{data.input.name}</label>
      <input {...data.input} type={data.type} step={data.step} required={data.required} placeholder={data.placeholder} id={`user_${data.input.name}`} className="form-control"/>
      {hasError && <span className="help-block" id={`user_${data.input.name}_helpBlock`}>{data.meta.error}</span>}
    </div>;
  }
